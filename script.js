...
// ========== CHECK TODAY - DISABLE BUTTONS PROPERLY ==========
        ...
        if (snap.exists()) {
            snap.forEach(child => {
                ...
                if (att.date === t && !att.checkOutTime) {
                    ...
                    // Today completed (checked in + out)
                    found = true;
                    todayAttendanceId = child.key;
                    ...
                    document.getElementById('checkTimeInfo').textContent = 
                        `In: ${fmtTime(att.checkInTime)} • Out: ${fmtTime(att.checkOutTime)}`;
                }
            });
        }
...
// ========== CHECK IN - FIXED: Disable immediately, prevent duplicates ==========
document.getElementById('checkInBtn').onclick = async () => {
    ...
    if (!auth.currentUser.emailVerified) {
        return toast('⚠️ Verify email first!');
    }
    ...
    try {
        // 🔍 Check if already checked in today (safety check)
        const snap = await db.ref('attendance/' + currentUser.uid + '/' + month).once('value');
        let alreadyCheckedIn = false;
        
        if (snap.exists()) {
            snap.forEach(child => {
                if (child.val().date === t && !child.val().checkOutTime) {
        ...
        if (alreadyCheckedIn) {
            toast('⚠️ Already checked in today!');
            await checkToday();
            isProcessing = false;
            return;
        }
        ...
        const name = userSnap.exists() ? userSnap.val().name : currentUser.email;
        ...
        document.getElementById('attendanceStatusBadge').innerHTML = '<span class="chip chip-success">Checked In</span>';
        outBtn.disabled = false;
        document.getElementById('checkTimeInfo').textContent = `In: ${fmtTime(now.toISOString())}`;
...
// ========== CHECK OUT - FIXED: Disable immediately ==========
document.getElementById('checkOutBtn').onclick = async () => {
    ...
    if (!todayAttendanceId) {
        return toast('⚠️ No check-in record found');
    }
    ...
    try {
        await db.ref('attendance/' + currentUser.uid + '/' + currentMonth() + '/' + todayAttendanceId).update({ 
            checkOutTime: new Date().toISOString() 
        });
        
        toast('🏁 Checked out! Have a great day!');
        
        // Update UI
        document.getElementById('attendanceStatusBadge').innerHTML = '<span class="chip chip-success">Completed</span>';
        document.getElementById('checkInBtn').disabled = true;
        document.getElementById('checkTimeInfo').textContent += ` • Out: ${fmtTime(new Date().toISOString())}`;
...
async function loadHistory() {
    ...
    const snap = await db.ref('attendance/' + uid).once('value');
    if (!snap.exists()) { 
    ...
    recent.forEach(r => {
        const monthLabel = r.date ? r.date.substring(0, 7) : 'Unknown';
        if (monthLabel !== currentMonthLabel) {
            currentMonthLabel = monthLabel;
            const monthName = new Date(r.date + 'T00:00:00').toLocaleDateString('en-US', { year: 'numeric', month: 'long' });
            list.innerHTML += `<div style="padding:12px 0 4px;font-weight:700;color:var(--primary);font-size:13px;">📅 ${monthName}</div>`;
        }
...
async function loadAdmin() {
    ...
    if (attSnap.exists()) {
        attSnap.forEach(userSnap => {
            userSnap.forEach(monthSnap => {
                monthSnap.forEach(daySnap => {
                    totalRecords++;
                    if (daySnap.val().date === t) todayCount++;
                });
            });
        });
    }
    ...
    document.getElementById('statTotalRecords').querySelector('.stat-num').textContent = totalRecords;
