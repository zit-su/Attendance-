...
document.getElementById('headerAdminBtn').onclick = () => {
    document.getElementById('navAdmin').click();
};
...
async function checkToday() {
    ...
    const snap = await db.ref('attendance').orderByChild('userId_date').equalTo(uid + '_' + t).once('value');
    
    const inBtn = document.getElementById('checkInBtn');
    const outBtn = document.getElementById('checkOutBtn');
    
    if (snap.exists()) {
        const d = Object.entries(snap.val())[0];
        todayAttendanceId = d[0];
        const att = d[1];
...
document.getElementById('checkInBtn').onclick = async () => {
    ...
    try {
        ...
        await ref.set({
            ...
            userId_date: currentUser.uid + '_' + t,
            userName: name,
...
async function loadHistory() {
    ...
    if (!snap.exists()) { list.innerHTML = '<div class="list-empty">No records yet</div>'; return; }
    const records = Object.values(snap.val()).sort((a, b) => (b.date || '').localeCompare(a.date || '')).slice(0, 30);
...
async function loadAdmin() {
    const [usersSnap, attSnap, todaySnap] = await Promise.all([
        db.ref('users').once('value'),
        db.ref('attendance').once('value'),
        db.ref('attendance').orderByChild('date').equalTo(today()).once('value')
    ]);
    ...
    if (!usersSnap.exists()) { ulist.innerHTML = '<div class="list-empty">No users</div>'; }
    else {
        ulist.innerHTML = '';
        usersSnap.forEach(c => {
            const u = c.val();
...
async function loadRecords() {
    ...
    if (!c) list.innerHTML = '<div class="list-empty">No matching records</div>';
}
...
document.getElementById('editUserForm').onsubmit = async (e) => {
    ...
    if (snap.exists()) {
        ...
        await db.ref('attendance/' + id).update({ type: document.getElementById('editUserType').value });
    }
    toast('✅ Updated!');
