...
async function updateIncome() {
    ...
    if (snap.exists()) {
        snap.forEach(day => {
            const d = day.val();
            if (d.checkInTime) {
                const amount = wage * getMultiplier(d.type || 'full-day');
                total += amount;
                if (d.date === todayStr) todayInc += amount;
            }
        });
    }
    document.getElementById('userMonthlyIncome').textContent = '₹' + total;
    document.getElementById('userTodayIncome').textContent = todayInc > 0 ? '₹' + todayInc : '--';
}
...
document.getElementById('checkInBtn').onclick = async () => {
    ...
    try {
        ...
        await checkToday(); await loadHistory(); await updateIncome();
...
async function loadAdmin() {
    ...
    ulist.innerHTML = '';
    // User dropdown for attendance form
    ...
    if (usersSnap.exists()) {
        usersSnap.forEach(c => {
            ...
            sel.innerHTML += `<option value="${uid}">${u.name} (${u.email})</option>`;
        });
    }
    loadRecords();
}
...
forms.adminAddAttendance.addEventListener('submit', async (e) => {
    ...
    if (!uid) return toast('Please select a user');
    if (!date) return toast('Please choose a date');
    const month = date.substring(0,7);
    const userSnap = await db.ref('users/' + uid).once('value');
    if (!userSnap.exists()) return toast('User not found');
    ...
    loadRecords();
    loadAdmin(); // Refresh dropdown in case new users were added? Not needed but safe.
});
