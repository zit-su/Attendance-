...
// ========== CHECK TODAY (NEW STRUCTURE: attendance/{uid}/{month}/...) ==========
async function checkToday() {
    const t = today();
    const month = currentMonth();
    const uid = currentUser.uid;
    
    // Read user's current month attendance
...
document.getElementById('checkInBtn').onclick = async () => {
    ...
    try {
        ...
        // NEW: attendance/{userId}/{year-month}/pushKey
...
async function loadHistory() {
    ...
    recent.forEach(r => {
        ...
        if (monthLabel !== currentMonthLabel) {
            ...
            const monthName = new Date(r.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long' });
...
async function loadAdmin() {
    ...
    // Iterate through new structure: attendance/{uid}/{month}/{key}
    if (attSnap.exists()) {
...
async function loadRecords() {
    ...
    snap.forEach(userSnap => {
        ...
        userSnap.forEach(monthSnap => {
            monthSnap.forEach(daySnap => {
                const r = daySnap.val();
                r._uid = uid; // Keep track of user ID
...
document.getElementById('editUserForm').onsubmit = async (e) => {
    ...
    // Update today's attendance type if needed
    const month = currentMonth();
    const t = today();
    ...
    if (snap.exists()) {
        snap.forEach(child => {
            if (child.val().date === t) {
                db.ref('attendance/' + uid + '/' + month + '/' + child.key).update({ type });
            }
        });
    }
